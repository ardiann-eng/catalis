...
// Daftar hostname untuk environment production
const productionHostnames = [
  "catalist.vercel.app",
  "www.catalist.vercel.app",
  "newcatalist.vercel.app",
  "catalist-creative.com",
  "www.catalist-creative.com"
];
  ...
  try {
    ...
    try {
      ...
      if (error.name === "AbortError") {
        ...
        if (isProduction) {
          throw new Error(
            "Layanan pembayaran sedang tidak tersedia. Silakan coba lagi nanti atau hubungi customer service."
          );
        } else {
          throw new Error(
...
async function getTokenFromBackend(requestData) {
  ...
  for (const backendUrl of backendUrls) {
    console.log("Mencoba koneksi ke backend di:", backendUrl);
    ...
    try {
      ...
      lastError = error;
      console.error(`Error saat menghubungi ${backendUrl}:`, error.message);
      
      // Lanjutkan ke URL berikutnya
      continue;
    }
  }
  
  // Jika semua URL gagal, lempar error terakhir
  if (lastError) {
    // Jika dalam mode production, berikan pesan error yang lebih user-friendly
    if (isProduction) {
      throw new Error("Layanan pembayaran sedang tidak tersedia. Silakan coba lagi nanti atau hubungi customer service.");
    } else {
      throw new Error("Gagal terhubung ke server backend: " + lastError.message);
    }
  } else {
    throw new Error("Tidak dapat terhubung ke server pembayaran");
  }
}
