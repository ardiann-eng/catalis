            ...
            case 'text':
                ...
                if (input.id === 'address') {
                    if (input.value.trim().length < 20) {
                        isValid = false;
                        errorMessage = 'Alamat minimal 20 karakter';
                    }
                }
                break;
        }
...
async function populateSavedAddresses() {
    try {
        ...
        const { data, error } = await supabase
            .from('user_addresses')
            .select('id, address_name, recipient_name, phone_number, full_address, city, postal_code, is_primary, created_at')
            .eq('user_id', user.id)
            .order('is_primary', { ascending: false })
            .order('created_at', { ascending: false });
        if (error) return;
        const addrs = data || [];
        select.innerHTML = '<option value="">Pilih alamat...</option>' + addrs.map((a) => {
            const label = `${a.address_name || 'Alamat'} • ${a.recipient_name} - ${a.city}`;
            return `<option value="${a.id}">${label}</option>`;
        }).join('');
        select.addEventListener('change', async () => {
            const id = select.value;
            if (id === '') return;
            const rec = addrs.find(x => String(x.id) === String(id)) || (await supabase.from('user_addresses').select('*').eq('id', id).single()).data;
            if (!rec) return;
            const fullnameInput = document.getElementById('fullname');
            const emailInput = document.getElementById('email');
            const phoneInput = document.getElementById('phone');
            ...
            const citySelect = document.getElementById('city');
            const districtSelect = document.getElementById('district');
            if (fullnameInput) fullnameInput.value = rec.recipient_name || '';
            if (phoneInput) phoneInput.value = rec.phone_number || '';
            if (addressInput) addressInput.value = rec.full_address || '';
            if (postalInput) postalInput.value = rec.postal_code || '';
            if (emailInput && user.email) emailInput.value = user.email;
            checkoutData.shipping.fullname = rec.recipient_name || '';
            checkoutData.shipping.phone = rec.phone_number || '';
            checkoutData.shipping.address = rec.full_address || '';
            checkoutData.shipping.postalCode = rec.postal_code || '';
            checkoutData.shipping.city = rec.city || '';
            resetSelect(citySelect);
            resetSelect(districtSelect, true);
            calculateShippingCost();
        });
        ...
        } else if (localStorage.getItem('selectedAddress')) {
